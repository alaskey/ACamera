...
public class CameraSettings {
    ...
    public static final String KEY_SHUTTERSOUND="pref_camera_shuttersound__key";
    ...
    public static final String KEY_RESTORE_DEFAULTS = "pref_camera_restore_defaults";
   ...
   public static List<String> getSupportedFlipMode(Parameters params){
    ...
    private void qcomInitPreferences(PreferenceGroup group){
        ...
        ListPreference shutterSound=group.findPreference(KEY_SHUTTERSOUND);
		if (shutterSound != null
				&& CameraActivity.CAMERA_SHUTTER_SOUND == false) {
			removePreference(group, shutterSound.getKey());
		}
        ...
        if (SystemProperties.getBoolean("persist.env.camera.saveinsd", false)) {
            ...
            if (pref != null) {
                savePathValue = pref.getString(KEY_CAMERA_SAVEPATH, "1");
            }
            if (savePath != null && "1".equals(savePathValue)) {
                // If sdCard is present, set sdCard as default save path.
                // Only for the first time when camera startt.
                if (SDCard.instance().isWriteable()) {
                    Log.d(TAG, "set sdcard as save path");
                    savePath.setValueIndex(1);
                } else {
                    Log.d(TAG, "set phone as save path when sdCard is unavailable");
                    savePath.setValueIndex(0);
                }
           }
        }
    }
    ...
    private static void upgradeOldVersion(SharedPreferences pref) {
        ...
        if (version == 1) {
            ...
            String defaultQuality = SystemProperties.get("persist.env.c.camera.quality", "85");
            String quality = pref.getString(KEY_JPEG_QUALITY, defaultQuality);
