...
public class Storage {
    ...
    public static void setSaveSDCard(boolean saveSDCard) {
        if (!isHaveExternalSDCard()) {//no sdcard available
            sSaveSDCard = false;
            Log.i(TAG, "haven't sdcard");
        }else{
            sSaveSDCard = saveSDCard;
            Log.i(TAG, "have sdcard");
        }
    }
    ...
    public static String generateFilepath(String title, String pictureFormat) {
        if (pictureFormat == null || pictureFormat.equalsIgnoreCase("jpeg")) {
            if (isSaveSDCard() && SDCard.instance().isWriteable() && isHaveExternalSDCard()) {
                File sDir = new File(SDCard.instance().getDirectory());
                if (!sDir.exists()) {
                    sDir.mkdirs();
                }
    ...
    private static long getInternalStorageAvailableSpace() {
        ...
        Log.d(TAG, "External storage state=" + state + ":" + DIRECTORY);
    ...
    public static boolean isHaveExternalSDCard() {
        ...
        for (int i = 0; i < volumes.length; i++) {
            if (volumes[i].isRemovable() && Environment.MEDIA_MOUNTED.equals(
                ...
                Log.v(TAG, Environment.MEDIA_MOUNTED + "===" + volumes[i].getPath());
                return true;
            }
        }
        return false;
