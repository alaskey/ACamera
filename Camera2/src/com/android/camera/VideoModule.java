...
public class VideoModule implements CameraModule,
    ...
    // if (mMediaRecorderPausing) return; // leo comment out [Bug63829]
        takeASnapshot();
    }
    ...
    public void onShutterButtonClick() {
		...
		ListPreference shutterpref = mPreferenceGroup
				.findPreference(CameraSettings.KEY_SHUTTERSOUND);
		if (shutterpref != null
				&& shutterpref.getValue().equals(
						CameraActivity.SHUTTER_SOUND_OFF)) {
			mCameraDevice.enableShutterSound(false);
		} else if (shutterpref != null
				&& shutterpref.getValue().equals(
						CameraActivity.SHUTTER_SOUND_ON)) {
    ...
    public void resizeForPreviewAspectRatio() {
        ...
        if (mProfile != null)
        mUI.setAspectRatio((double) mProfile.videoFrameWidth * 1.0 / mProfile.videoFrameHeight);
    }
    ...
    public void onResumeAfterSuper() {
        mUI.enableShutter(false);
        mUI.requestLayoutRecordingTimeRect();
        mZoomValue = 0;
    ...
    private String getVideoFilename(int outputFileFormat){
    ...
    private void generateVideoFilename(int outputFileFormat) {
        ...
        String path = getVideoFilename(outputFileFormat);
    ...
    private void saveVideo() {
        if (mVideoFileDescriptor == null) {
            ...
            mActivity.getMediaSaveService().addVideo(mCurrentVideoFilename,
                    ...
                    getVideoFilename(mProfile.fileFormat)
            );
        }
        mCurrentVideoValues = null;
    }
    ...
    private void startVideoRecording() {
        if(!mPreviewing) {
            Log.d(TAG, "startVideoRecording failed, because startpreview has not stated");
            return;
        }
