...
public class CameraActivity extends Activity
    ...
    public static boolean CAMERA_SHUTTER_SOUND=false;
    public static final String SHUTTER_SOUND_OFF="disable";
    public static final String SHUTTER_SOUND_ON="enable";
    ...
    private static final int SUPPORT_ROTATE = 0;//1 << 1;
    ...
    //For detailsDialog dismissing when homeKey down.
    private Dialog mDetailsDialog = null;
            ...
            new FilmStripView.Listener() {
                ...
                public void onDataPromoted(int dataID) {
                    ...
                    if (currentDataId > dataID) {
                        mFilmStripView.getController().goToFrontItem();
                    } else if (currentDataId < dataID){
                ...
                public void onDataFullScreenChange(int dataID, boolean full) {
                    ...
                    } else {//now is in camera full preview, check PreviewControls visible again
                        if(full && !arePreviewControlsVisible()) {
                ...
                public void onCurrentDataOffCentered(int dataID) {
                    //by songbai add mFilmStripView.isCameraPreview() for fix the bug 73190
                    if ((dataID != 0 && !mFilmStripView.isCameraPreview()) || mFilmStripView.isCameraPreview()) {
                        // For now, We ignore all items that are not the camera preview.
                        // And ingnore if it is still in camera preview.
                        return;
                    }
...
//        mFilmStripView.getController().goToNextItem();
    ...
    private void removeData(final int dataID) {
...
//                            DetailsDialog.create(CameraActivity.this, mediaDetails).show();
                            mDetailsDialog = DetailsDialog.create(CameraActivity.this, mediaDetails);
                            mDetailsDialog.show();
                        }
                    }
                }).execute();
    ...
    public void onCreate(Bundle state) {
    	...
    	Log.v("SKY", "SKY in oncreate");
    	
        super.onCreate(state);
        GcamHelper.init(getContentResolver());

        CAMERA_SHUTTER_SOUND=SystemProperties.getBoolean("persist.env.camera.shutsound", false);
...
//        }
    ...
    public void onConfigurationChanged(Configuration config) {
        ...
        //Remove by songbai. This modifications maby cause another bug 75140.
        //Fix bug 73190 in another way. Please pay attention to onCurrentDataOffCentered.
        //added by longyu for bug 73190
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (mFilmStripView.inCameraFullscreen() && !mIsModuleSwitchInProgress) {
    ...
    public void onBackPressed() {
        if(mCurrentModule instanceof PhotoModule){//bug-id:64020
            PhotoModule photoModule = (PhotoModule)mCurrentModule;
            if(photoModule.isCountingDown()){
                photoModule.cancelCountDown();
                photoModule.setMenuAndSwitcherEnable();
                return;
            }
        }
        if (!mFilmStripView.inCameraFullscreen()) {
...
//            message = getString(R.string.access_sd_fail);
            ...
            if(Storage.isSaveSDCard()){//sd card
    ...
    private void setModuleFromIndex(int moduleIndex) {
        ...
        switch (moduleIndex) {
            ...
            case ModuleSwitcher.WIDE_ANGLE_PANO_MODULE_INDEX:
                ...
                popMessage(getResources().getString(R.string.panorama_guide_shutter));
                break;
    ...
    private boolean mIsMessageShowing = false;
    public void popMessage(final String msg){
        if(!mIsMessageShowing){
            mIsMessageShowing = true;
            Toast.makeText(CameraActivity.this, msg, Toast.LENGTH_LONG).show();
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    mIsMessageShowing = false;
                }
            }, 3000);
        }
    }
    ...
    // For get current interface status.
