...
public class PhotoUI implements PieListener,
    ...
    private TextView mContinus; // leo add
    ...
    private DecodeTask mShowPreviewThumbTask;
    ...
    // for swap between camera and built-in gallery
    private DisplayMetrics mDm;
    private float mRatio;
    private boolean mPreviewFocused;
    private OrientationManager mOrientationManager;
    private boolean isFaceDetectionOpen = true;
...
//            }
    ...
    public PhotoUI(CameraActivity activity, PhotoController controller, View parent) {
        ...
        mContinus = (TextView)mRootView.findViewById(R.id.continous_num);
    ...
    public void setConinousNum(int i) {
        if(i > 0) {
            if(i < 3) {
                mContinus.setVisibility(View.VISIBLE);
            }
            mContinus.setText(i+"");
            return;
        }
        mContinus.setVisibility(View.INVISIBLE);
    }
    ...
    public void setAspectRatio(float ratio) {
        ...
        //comment out force update leo.wang
        //if (mAspectRatio != ratio) {
            ...
            int h;
            if(mPreviewFocused) {
                if (mPopup != null || mDismissAll == false) {
    ...
    void getScreenSize() {
        ...
        Log.v(TAG,"widthPixels:"+mDm.widthPixels+",heightPixels:"+mDm.heightPixels);
    }
    ...
    public void animateCapture(final byte[] jpegData, int orientation, boolean mirror) {
        ...
        if(mShowPreviewThumbTask != null && mShowPreviewThumbTask.getStatus() == Status.RUNNING){
            mShowPreviewThumbTask.cancel(true);
        }
        mShowPreviewThumbTask = null;
        mShowPreviewThumbTask = new DecodeTask(jpegData, orientation, mirror);
        mShowPreviewThumbTask.execute();
    }
    ...
    public void setMenuButtonEnable(Boolean enable){
        if(enable){
            ...
            mOnScreenIndicators.setIconEnabledAlpha(true);
        }else{
            mMenuButton.setEnabled(false);
            mOnScreenIndicators.setIconEnabledAlpha(false);
//            setShowMenu(false);
        }

    }

    public void setSwitcherEnable(Boolean enable){
        if(enable){
    ...
    public void disableSettings(final String ... keyvalues) {
        if (mMenu == null) return;
        mMenu.overrideSettings(keyvalues);
        mMenu.disableSettings(keyvalues);
    }
        ...
        if (mPopup == null) {
            ...
            mPopup.setOnDismissListener(new PopupWindow.OnDismissListener() {
                ...
                public void onDismiss() {
                    ...
                    mDismissAll = false;
                    if (mMenu.isPopupDismissAll()) {
                        //for fix the bug 74200, show camera ui only in preview interface.
                        if(mPreviewFocused ||  mActivity.isCameraPreview()) {
                            showUI();
                        }
                    }
                    ...
                    ((CameraActivity)mActivity).updateStorageSpaceAndHint();
                }
            });
        }
    ...
    public void dismissRestoreDefaults(boolean dismissAll) {
        mMenu.popupDismissed(dismissAll);
    }
    ...
    public void setOrientationManager(OrientationManager o) {
        mOrientationManager = o;
    }

    public void setFaceDetectionOpen(boolean isOpen){
        isFaceDetectionOpen = isOpen;
    }

    public void onOrientationChanged(int orientation) {
        mPreviewThumb.setRotation(-orientation);
