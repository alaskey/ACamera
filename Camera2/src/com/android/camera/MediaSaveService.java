...
public class MediaSaveService extends Service {
    ...
    public void addVideo(String path, long duration, ContentValues values,
                         OnMediaSavedListener l, ContentResolver resolver, String newPath) {
        // We don't set a queue limit for video saving because the file
        // is already in the storage. Only updating the database.
        new VideoSaveTask(path, duration, values, l, resolver, newPath).execute();
//        new VideoSaveTask(path, duration, values, l, resolver).execute();
    ...
    private class VideoSaveTask extends AsyncTask <Void, Void, Uri> {
        ...
        public VideoSaveTask(String path, long duration, ContentValues values,
                             OnMediaSavedListener l, ContentResolver r, String newPath) {
            this(path, duration, values, l, r);
        ...
        private String createName(long dateTaken) {
            ...
            SimpleDateFormat dateFormat = new SimpleDateFormat(
                    MediaSaveService.this.getString(R.string.video_file_name_format));

            return dateFormat.format(date);
        }
        @Override
        protected Uri doInBackground(Void... v) {
            if(newPath != null){//copy file.
...
//                    deleteVideoFile(path);
                    ...
                    path = newPath;
                    values.remove(Video.Media.TITLE);
                    values.remove(Video.Media.DATA);
                    values.remove(Video.Media.DATE_TAKEN);
